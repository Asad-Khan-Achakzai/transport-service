import { Injectable } from '@angular/core';
import { AuthService } from '../core/auth.service';
import { HttpClient } from '@angular/common/http';
import { Observable } from 'rxjs';
import { Path } from '../server.config';
import * as pdfFonts from 'pdfmake/build/vfs_fonts';
import { NavController, Platform } from '@ionic/angular';
import * as pdfmake from 'pdfmake/build/pdfmake';
import { serviceProvider } from 'src/app/customer-dashboard/service-provider.model';
import { Storage } from '@ionic/storage';
import { DatePipe } from '@angular/common';
import { CustomersService } from './customers.service';
import { File } from '@ionic-native/file/ngx';
import { FileOpener } from '@ionic-native/file-opener/ngx';

@Injectable({
    providedIn: 'root'
})
export class BookingsService {
    bookingCustomer: string;
    bookedSeats: Array<string>;
    invoiceData: Booking;
    invoiceCustmer: customer;
    invoiceServiceProvider: serviceProvider;
    today: number = Date.now();
    date = this.datepipe.transform(this.today, 'longDate');
    time = this.datepipe.transform(this.today, 'shortTime');
    pdfObj = null;
  
    constructor(private plt: Platform,private file: File, private fileOpener: FileOpener,private customerService: CustomersService,public datepipe: DatePipe, private authService: AuthService, private http: HttpClient, private storage: Storage) { }
    public saveBookingId(id: string) {
        this.storage.set('bookingId', id);
    }
    public async getBookingId() {
        let id;
        await this.storage.get('bookingId').then((val) => {
            console.log('id in funciton =', val);
            id = val;
        });
        //const id = await this.storage.get('serviceProviderId');
        return id;
    }
    public saveBookingForEdit(object: Booking) {
        this.storage.set('booking', object);
    }
    public async getBookingForEdit() {
        let id;
        await this.storage.get('booking').then((val) => {
            console.log('id in funciton =', val);
            id = val;
        });
        //const id = await this.storage.get('serviceProviderId');
        return id;
    }
    public saveCustomerIdForServiceProvider(id) {
        this.storage.set('custmerIdBooking', id);
    }
    public async getCustomerIdForServiceProvider() {
        let id;
        await this.storage.get('custmerIdBooking').then((val) => {
            console.log('id in funciton =', val);
            id = val;
        });
        //const id = await this.storage.get('serviceProviderId');
        return id;
    }

    public saveBookingroute(route:object) {
        this.storage.set('bookingRoute', route);
    }
    public async getBookingroute() {
        let id;
        await this.storage.get('bookingRoute').then((val) => {
            console.log('id in funciton =', val);
            id = val;
        });
        //const id = await this.storage.get('serviceProviderId');
        return id;
    }

    putBookingCustomer(id) {
        this.bookingCustomer = id;
    }
    public getBookingCustomer(id): Observable<any> {
       // console.log('customer id =', this.bookingCustomer);
        const url = Path.getPath() + `/users/${id}`;

        return this.http.get(url);
    }
    
    public bookingRegister(credentials: Booking): Observable<any> {
        const url = Path.getPath() + '/bookings/registerBooking';

        return this.http.post(url, credentials);
    }
    public deleteBooking(_idd): Observable<any> {
        console.log('id =', _idd);
        const url = Path.getPath() + `/bookings/${_idd}`;

        return this.http.delete(url);
    }
    public getBooking(id): Observable<any> {
        console.log('id =', id);
        const url = Path.getPath() + `/bookings/${id}`;

        return this.http.get(url);
    }
    public getBookingObject(_id): Observable<any> {
        //console.log('id =',id);
        const url = Path.getPath() + `/bookings/${_id}`;
        let object: {
            name: 'xyz'
        }
        return this.http.post(url, object);
    }

    public getRouteBookings(id): Observable<any> {
        console.log('id =', id);
        const url = Path.getPath() + `/bookings/routeBookings/${id}`;

        return this.http.get(url);
    }
    public putBookedSeats(seats) {
        this.bookedSeats = seats;
    }
    putInvoiceData(data) {
        this.invoiceData = data;
        console.log('invoice data =', this.invoiceData);
        this.getInvoiceCustomer(this.invoiceData.customerId);
        this.getInvoiceServiceProvider(this.invoiceData.providerId);
    }
    async fillUserData(customerId, providerId) {
        await this.getInvoiceCustomer(customerId)
        await this.getInvoiceServiceProvider(providerId);
    }
    public expireBooking(routeId): Observable<any> {
        let object ={
            'id': routeId
        };
        console.log('id =', routeId);
        const url = Path.getPath() + `/bookings/${routeId}`;

        return this.http.put(url,object);
    }
    async getInvoiceCustomer(id) {
        const url = Path.getPath() + `/users/${id}`;
        const observable = await this.http.get(url);
        observable.subscribe(
            (data: any) => {
                this.invoiceCustmer = data.data;
                //console.log('object = ',data);
                console.log('customer = ', this.invoiceCustmer);
                // this.id = this.costomerInfo._id;
                // this.email = this.costomerInfo.email;
                // this.userName = this.costomerInfo.username;
                // this.phone = this.costomerInfo.phone;
                // this.cnic = this.costomerInfo.cnic;
                // this.customerService.customerName = this.costomerInfo.username;
                // this.customerService.logedInCustomerId = this.costomerInfo._id
                //console.log('data', data.data);
            },
            err => {
                console.log('err', err);
            },

        );
    }
    async getInvoiceServiceProvider(id) {
        const url = Path.getPath() + `/providers/${id}`;
        const observable = await this.http.get(url);
        observable.subscribe(
            (data: any) => {
                this.invoiceServiceProvider = data.data;
                //console.log('object = ',data);
                console.log('invoiceServiceProvider = ', this.invoiceServiceProvider);
                // this.id = this.costomerInfo._id;
                // this.email = this.costomerInfo.email;
                // this.userName = this.costomerInfo.username;
                // this.phone = this.costomerInfo.phone;
                // this.cnic = this.costomerInfo.cnic;
                // this.customerService.customerName = this.costomerInfo.username;
                // this.customerService.logedInCustomerId = this.costomerInfo._id
                //console.log('data', data.data);
            },
            err => {
                console.log('err', err);
            },

        );
    }
    async makePdf(booking: Booking) {
let price = await this.customerService.getRoutePrice();
        pdfmake.vfs = pdfFonts.pdfMake.vfs;
        var dd = {



            footer: {
                columns: [
                    {

                        text: 'Contact Company:\n' +' '+ this.invoiceServiceProvider.phone,
                        style: 'documentFooterLeft',
                        width: '*'



                    },
                    {
                        text: '',
                        style: 'documentFooterCenter'
                    },
                    { text: '', style: 'documentFooterRight' }
                ]
            },
            content: [
                // Header
                {
                    columns: [
                       
                        [
                            {
                                text: 'INVOICE',
                                style: 'invoiceTitle',
                                width: '*'
                            },
                            {
                                stack: [
                                    {
                                        columns: [
                                            {
                                                text: 'Booking ID',
                                                style: 'invoiceSubTitle',
                                                width: '*'

                                            },
                                            {
                                                text: booking.id,
                                                style: 'invoiceSubValue',
                                                width: 100

                                            }
                                        ]
                                    },
                                    {
                                        columns: [
                                            {
                                                text: 'Date Issued',
                                                style: 'invoiceSubTitle',
                                                width: '*'
                                            },
                                            {
                                                text: this.date,
                                                style: 'invoiceSubValue',
                                                width: 100
                                            }
                                        ]
                                    },
                                    {
                                        columns: [
                                            {
                                                text: 'Time issued',
                                                style: 'invoiceSubTitle',
                                                width: '*'
                                            },
                                            {
                                                // text:'June 05, 2016',
                                                text: this.time,
                                                style: 'invoiceSubValue',
                                                width: 100
                                            }
                                        ]
                                    }, {
                                        columns: [
                                            {
                                                text: 'Departure Time',
                                                style: 'invoiceSubTitle',
                                                width: '*'
                                            },
                                            {
                                                // text:'June 05, 2016',
                                                text: booking.timing,
                                                style: 'invoiceSubValue',
                                                width: 100
                                            }
                                        ]
                                    },
                                ]
                            }
                        ],
                    ],
                },
                // Billing Headers
                {
                    columns: [
                        {
                            text: 'Billing From',
                            style: 'invoiceBillingTitle',

                        },
                        {
                            text: 'Billing To',
                            style: 'invoiceBillingTitle',

                        },
                    ]
                },
                // Billing Details
                {
                    columns: [
                        {
                            //  text: 'Your Name \n Your Company Inc.',
                            text: this.invoiceServiceProvider.companyName,
                            style: 'invoiceBillingDetails'
                        },
                        {
                            //  text: 'Client Name \n Client Company',
                            text: this.invoiceCustmer.username,
                            style: 'invoiceBillingDetails'
                        },
                    ]
                },
                // Billing Address Title
                {
                    columns: [
                        {
                            text: 'Address',
                            style: 'invoiceBillingAddressTitle'
                        },
                        {
                            text: 'phone#',
                            style: 'invoiceBillingAddressTitle'
                        },
                    ]
                },
                // Billing Address
                {
                    columns: [
                        {
                            //  text: '9999 Street name 1A \n New-York City NY 00000 \n   USA',
                            text: this.invoiceServiceProvider.officeLocation,
                            style: 'invoiceBillingAddress'
                        },
                        {
                            //  text: '1111 Other street 25 \n New-York City NY 00000 \n   USA',
                            text: this.invoiceCustmer.phone,
                            style: 'invoiceBillingAddress'
                        },
                    ]
                },
                // Line breaks
                '\n\n',
                // Items
                {
                    table: {
                        // headers are automatically repeated if the table spans over multiple pages
                        // you can declare how many rows should be treated as headers
                        headerRows: 1,
                        widths: ['*', 40, 'auto', 40, 'auto', 80],

                        body: [
                            // Table Header
                            [
                                {
                                    text: 'Seats',
                                    style: 'itemsHeader'
                                },
                                {
                                    text: 'Qty',
                                    style: ['itemsHeader', 'center']
                                },
                                {
                                    text: 'Price',
                                    style: ['itemsHeader', 'center']
                                },
                                {
                                    text: 'Tax',
                                    style: ['itemsHeader', 'center']
                                },
                                {
                                    text: 'Discount',
                                    style: ['itemsHeader', 'center']
                                },
                                {
                                    text: 'Total',
                                    style: ['itemsHeader', 'center']
                                }
                            ],
                            // Items
                            // Item 1
                            [
                                [
                                    {
                                        //  text: 'Item 1',
                                        text: booking.seats,
                                        style: 'itemTitle'
                                    }
                                ],
                                {
                                    //  text:'1',
                                    text: booking.seats.length,
                                    style: 'itemNumber'
                                },
                                {
                                    //text: '$999.99',
                                    text: '$'+price,
                                    style: 'itemNumber'
                                },
                                {
                                    text: '0%',
                                    style: 'itemNumber'
                                },
                                {
                                    text: '0%',
                                    style: 'itemNumber'
                                },
                                {
                                    //  text: '$999.99',
                                    text: '$'+booking.price,
                                    style: 'itemTotal'
                                }
                            ],
                            // Item 2
                            //  [ 
                            //      [
                            //          {
                            //              text: 'Item 2',
                            //              style:'itemTitle'
                            //          }, 
                            //          {
                            //              text: 'Item Description',
                            //              style:'itemSubTitle'

                            //          }
                            //      ], 
                            //      {
                            //          text:'1',
                            //          style:'itemNumber'
                            //      }, 
                            //      {
                            //          text:'$999.99',
                            //          style:'itemNumber'
                            //      }, 
                            //      {
                            //          text:'0%',
                            //          style:'itemNumber'
                            //      }, 
                            //      {
                            //          text: '0%',
                            //          style:'itemNumber'
                            //      },
                            //      {
                            //          text: '$999.99',
                            //          style:'itemTotal'
                            //      } 
                            //  ],
                            // END Items
                        ]
                    }, // table
                    //  layout: 'lightHorizontalLines'
                },
                // TOTAL
                {
                    table: {
                        // headers are automatically repeated if the table spans over multiple pages
                        // you can declare how many rows should be treated as headers
                        headerRows: 0,
                        widths: ['*', 80],

                        body: [
                            // Total
                           
                            [
                                {
                                    text: 'Tax 5%',
                                    style: 'itemsFooterSubTitle'
                                },
                                {
                                    text: '$'+booking.price/100*5,
                                    style: 'itemsFooterSubValue'
                                }
                            ],
                            [
                                {
                                    text: ' Grand Total',
                                    style: 'itemsFooterTotalTitle'
                                },
                                {
                                    //  text: '$2523.93',
                                    text:'$'+(booking.price + booking.price/100*5),
                                    style: 'itemsFooterTotalValue'
                                }
                            ],
                        ]
                    }, // table
                    layout: 'lightHorizontalLines'
                },
                // Signature
                {
                    columns: [
                        {
                            text: '',
                        },
                        {
                            stack: [
                                {
                                    text: '_________________________________',
                                    style: 'signaturePlaceholder'
                                },
                                {
                                    //  text: 'Your Name',
                                    text: this.invoiceServiceProvider.companyName,
                                    style: 'signatureName'

                                },
                                {
                                    //  text: 'Your job title',
                                    text: 'Manager',
                                    style: 'signatureJobTitle'

                                }
                            ],
                            width: 180
                        },
                    ]
                },
                {
                    text: 'NOTES',
                    style: 'notesTitle'
                },
                {
                    text: 'Reach terminal 15 minutes before departure time. \n Before begining your journey sign the invoice from the manager in terminal. \n If there is any problem contact with company contact number down in invoice. \n enjoy your journey :). ',
                    style: 'notesText'
                }
            ],
            styles: {
                // Document Header
                documentHeaderLeft: {
                    fontSize: 10,
                    margin: [5, 5, 5, 5],
                    alignment: 'left'
                },
                documentHeaderCenter: {
                    fontSize: 10,
                    margin: [5, 5, 5, 5],
                    alignment: 'center'
                },
                documentHeaderRight: {
                    fontSize: 10,
                    margin: [5, 5, 5, 5],
                    alignment: 'right'
                },
                // Document Footer
                documentFooterLeft: {
                    fontSize: 10,
                    margin: [5, 5, 5, 5],
                    alignment: 'left'
                },
                documentFooterCenter: {
                    fontSize: 10,
                    margin: [5, 5, 5, 5],
                    alignment: 'center'
                },
                documentFooterRight: {
                    fontSize: 10,
                    margin: [5, 5, 5, 5],
                    alignment: 'right'
                },
                // Invoice Title
                invoiceTitle: {
                    fontSize: 22,
                    bold: true,
                    alignment: 'right',
                    margin: [0, 0, 0, 15]
                },
                // Invoice Details
                invoiceSubTitle: {
                    fontSize: 12,
                    alignment: 'right'
                },
                invoiceSubValue: {
                    fontSize: 12,
                    alignment: 'right'
                },
                // Billing Headers
                invoiceBillingTitle: {
                    fontSize: 14,
                    bold: true,
                    alignment: 'left',
                    margin: [0, 20, 0, 5],
                },
                // Billing Details
                invoiceBillingDetails: {
                    alignment: 'left'

                },
                invoiceBillingAddressTitle: {
                    margin: [0, 7, 0, 3],
                    bold: true
                },
                invoiceBillingAddress: {

                },
                // Items Header
                itemsHeader: {
                    margin: [0, 5, 0, 5],
                    bold: true
                },
                // Item Title
                itemTitle: {
                    bold: true,
                },
                itemSubTitle: {
                    italics: true,
                    fontSize: 11
                },
                itemNumber: {
                    margin: [0, 5, 0, 5],
                    alignment: 'center',
                },
                itemTotal: {
                    margin: [0, 5, 0, 5],
                    bold: true,
                    alignment: 'center',
                },

                // Items Footer (Subtotal, Total, Tax, etc)
                itemsFooterSubTitle: {
                    margin: [0, 5, 0, 5],
                    bold: true,
                    alignment: 'right',
                },
                itemsFooterSubValue: {
                    margin: [0, 5, 0, 5],
                    bold: true,
                    alignment: 'center',
                },
                itemsFooterTotalTitle: {
                    margin: [0, 5, 0, 5],
                    bold: true,
                    alignment: 'right',
                },
                itemsFooterTotalValue: {
                    margin: [0, 5, 0, 5],
                    bold: true,
                    alignment: 'center',
                },
                signaturePlaceholder: {
                    margin: [0, 70, 0, 0],
                },
                signatureName: {
                    bold: true,
                    alignment: 'center',
                },
                signatureJobTitle: {
                    italics: true,
                    fontSize: 10,
                    alignment: 'center',
                },
                notesTitle: {
                    fontSize: 10,
                    bold: true,
                    margin: [0, 50, 0, 3],
                },
                notesText: {
                    fontSize: 10
                },
                center: {
                    alignment: 'center',
                },
            },
            defaultStyle: {
                columnGap: 20,
            }
        }
        this.pdfObj = pdfmake.createPdf(dd);
        this.downloadPdf();
      //  pdfmake.createPdf(dd).open();
        

    }
    async makePdfOfManual(customerName, pricePerSeat, customerPhone, date, seats, price,routeTiming) {
       // let price = await this.customerService.getRoutePrice();
                pdfmake.vfs = pdfFonts.pdfMake.vfs;
                var dd = {
        
        
        
                    footer: {
                        columns: [
                            {
        
                                text: 'Contact Company:\n' +' '+ this.invoiceServiceProvider.phone,
                                style: 'documentFooterLeft',
                                width: '*'
        
        
        
                            },
                            {
                                text: '',
                                style: 'documentFooterCenter'
                            },
                            { text: '', style: 'documentFooterRight' }
                        ]
                    },
                    content: [
                        // Header
                        {
                            columns: [
                               
                                [
                                    {
                                        text: 'INVOICE',
                                        style: 'invoiceTitle',
                                        width: '*'
                                    },
                                    {
                                        stack: [
                                            // {
                                            //     columns: [
                                            //         {
                                            //             text: 'Booking ID',
                                            //             style: 'invoiceSubTitle',
                                            //             width: '*'
        
                                            //         },
                                            //         {
                                            //             text: booking.id,
                                            //             style: 'invoiceSubValue',
                                            //             width: 100
        
                                            //         }
                                            //     ]
                                            // },
                                            {
                                                columns: [
                                                    {
                                                        text: 'Date Issued',
                                                        style: 'invoiceSubTitle',
                                                        width: '*'
                                                    },
                                                    {
                                                        text: this.date,
                                                        style: 'invoiceSubValue',
                                                        width: 100
                                                    }
                                                ]
                                            },
                                            {
                                                columns: [
                                                    {
                                                        text: 'Time issued',
                                                        style: 'invoiceSubTitle',
                                                        width: '*'
                                                    },
                                                    {
                                                        // text:'June 05, 2016',
                                                        text: this.time,
                                                        style: 'invoiceSubValue',
                                                        width: 100
                                                    }
                                                ]
                                            }, {
                                                columns: [
                                                    {
                                                        text: 'Departure Time',
                                                        style: 'invoiceSubTitle',
                                                        width: '*'
                                                    },
                                                    {
                                                        // text:'June 05, 2016',
                                                        text: routeTiming,
                                                        style: 'invoiceSubValue',
                                                        width: 100
                                                    }
                                                ]
                                            },
                                        ]
                                    }
                                ],
                            ],
                        },
                        // Billing Headers
                        {
                            columns: [
                                {
                                    text: 'Billing From',
                                    style: 'invoiceBillingTitle',
        
                                },
                                {
                                    text: 'Billing To',
                                    style: 'invoiceBillingTitle',
        
                                },
                            ]
                        },
                        // Billing Details
                        {
                            columns: [
                                {
                                    //  text: 'Your Name \n Your Company Inc.',
                                    text: this.invoiceServiceProvider.companyName,
                                    style: 'invoiceBillingDetails'
                                },
                                {
                                    //  text: 'Client Name \n Client Company',
                                    text: customerName,
                                    style: 'invoiceBillingDetails'
                                },
                            ]
                        },
                        // Billing Address Title
                        {
                            columns: [
                                {
                                    text: 'Address',
                                    style: 'invoiceBillingAddressTitle'
                                },
                                {
                                    text: 'phone#',
                                    style: 'invoiceBillingAddressTitle'
                                },
                            ]
                        },
                        // Billing Address
                        {
                            columns: [
                                {
                                    //  text: '9999 Street name 1A \n New-York City NY 00000 \n   USA',
                                    text: this.invoiceServiceProvider.officeLocation,
                                    style: 'invoiceBillingAddress'
                                },
                                {
                                    //  text: '1111 Other street 25 \n New-York City NY 00000 \n   USA',
                                    text: customerPhone,
                                    style: 'invoiceBillingAddress'
                                },
                            ]
                        },
                        // Line breaks
                        '\n\n',
                        // Items
                        {
                            table: {
                                // headers are automatically repeated if the table spans over multiple pages
                                // you can declare how many rows should be treated as headers
                                headerRows: 1,
                                widths: ['*', 40, 'auto', 40, 'auto', 80],
        
                                body: [
                                    // Table Header
                                    [
                                        {
                                            text: 'Seats',
                                            style: 'itemsHeader'
                                        },
                                        {
                                            text: 'Qty',
                                            style: ['itemsHeader', 'center']
                                        },
                                        {
                                            text: 'Price',
                                            style: ['itemsHeader', 'center']
                                        },
                                        {
                                            text: 'Tax',
                                            style: ['itemsHeader', 'center']
                                        },
                                        {
                                            text: 'Discount',
                                            style: ['itemsHeader', 'center']
                                        },
                                        {
                                            text: 'Total',
                                            style: ['itemsHeader', 'center']
                                        }
                                    ],
                                    // Items
                                    // Item 1
                                    [
                                        [
                                            {
                                                //  text: 'Item 1',
                                                text: seats,
                                                style: 'itemTitle'
                                            }
                                        ],
                                        {
                                            //  text:'1',
                                            text: seats.length,
                                            style: 'itemNumber'
                                        },
                                        {
                                            //text: '$999.99',
                                            text: '$'+pricePerSeat,
                                            style: 'itemNumber'
                                        },
                                        {
                                            text: '0%',
                                            style: 'itemNumber'
                                        },
                                        {
                                            text: '0%',
                                            style: 'itemNumber'
                                        },
                                        {
                                            //  text: '$999.99',
                                            text: '$'+price,
                                            style: 'itemTotal'
                                        }
                                    ],
                                    // Item 2
                                    //  [ 
                                    //      [
                                    //          {
                                    //              text: 'Item 2',
                                    //              style:'itemTitle'
                                    //          }, 
                                    //          {
                                    //              text: 'Item Description',
                                    //              style:'itemSubTitle'
        
                                    //          }
                                    //      ], 
                                    //      {
                                    //          text:'1',
                                    //          style:'itemNumber'
                                    //      }, 
                                    //      {
                                    //          text:'$999.99',
                                    //          style:'itemNumber'
                                    //      }, 
                                    //      {
                                    //          text:'0%',
                                    //          style:'itemNumber'
                                    //      }, 
                                    //      {
                                    //          text: '0%',
                                    //          style:'itemNumber'
                                    //      },
                                    //      {
                                    //          text: '$999.99',
                                    //          style:'itemTotal'
                                    //      } 
                                    //  ],
                                    // END Items
                                ]
                            }, // table
                            //  layout: 'lightHorizontalLines'
                        },
                        // TOTAL
                        {
                            table: {
                                // headers are automatically repeated if the table spans over multiple pages
                                // you can declare how many rows should be treated as headers
                                headerRows: 0,
                                widths: ['*', 80],
        
                                body: [
                                    // Total
                                   
                                    [
                                        {
                                            text: 'Tax 5%',
                                            style: 'itemsFooterSubTitle'
                                        },
                                        {
                                            text: '$'+price/100*5,
                                            style: 'itemsFooterSubValue'
                                        }
                                    ],
                                    [
                                        {
                                            text: ' Grand Total',
                                            style: 'itemsFooterTotalTitle'
                                        },
                                        {
                                            //  text: '$2523.93',
                                            text:'$'+(price + price/100*5),
                                            style: 'itemsFooterTotalValue'
                                        }
                                    ],
                                ]
                            }, // table
                            layout: 'lightHorizontalLines'
                        },
                        // Signature
                        {
                            columns: [
                                {
                                    text: '',
                                },
                                {
                                    stack: [
                                        {
                                            text: '_________________________________',
                                            style: 'signaturePlaceholder'
                                        },
                                        {
                                            //  text: 'Your Name',
                                            text: this.invoiceServiceProvider.companyName,
                                            style: 'signatureName'
        
                                        },
                                        {
                                            //  text: 'Your job title',
                                            text: 'Manager',
                                            style: 'signatureJobTitle'
        
                                        }
                                    ],
                                    width: 180
                                },
                            ]
                        },
                        {
                            text: 'NOTES',
                            style: 'notesTitle'
                        },
                        {
                            text: 'Reach terminal 15 minutes before departure time. \n Before begining your journey sign the invoice from the manager in terminal. \n If there is any problem contact with company contact number down in invoice. \n enjoy your journey :). ',
                            style: 'notesText'
                        }
                    ],
                    styles: {
                        // Document Header
                        documentHeaderLeft: {
                            fontSize: 10,
                            margin: [5, 5, 5, 5],
                            alignment: 'left'
                        },
                        documentHeaderCenter: {
                            fontSize: 10,
                            margin: [5, 5, 5, 5],
                            alignment: 'center'
                        },
                        documentHeaderRight: {
                            fontSize: 10,
                            margin: [5, 5, 5, 5],
                            alignment: 'right'
                        },
                        // Document Footer
                        documentFooterLeft: {
                            fontSize: 10,
                            margin: [5, 5, 5, 5],
                            alignment: 'left'
                        },
                        documentFooterCenter: {
                            fontSize: 10,
                            margin: [5, 5, 5, 5],
                            alignment: 'center'
                        },
                        documentFooterRight: {
                            fontSize: 10,
                            margin: [5, 5, 5, 5],
                            alignment: 'right'
                        },
                        // Invoice Title
                        invoiceTitle: {
                            fontSize: 22,
                            bold: true,
                            alignment: 'right',
                            margin: [0, 0, 0, 15]
                        },
                        // Invoice Details
                        invoiceSubTitle: {
                            fontSize: 12,
                            alignment: 'right'
                        },
                        invoiceSubValue: {
                            fontSize: 12,
                            alignment: 'right'
                        },
                        // Billing Headers
                        invoiceBillingTitle: {
                            fontSize: 14,
                            bold: true,
                            alignment: 'left',
                            margin: [0, 20, 0, 5],
                        },
                        // Billing Details
                        invoiceBillingDetails: {
                            alignment: 'left'
        
                        },
                        invoiceBillingAddressTitle: {
                            margin: [0, 7, 0, 3],
                            bold: true
                        },
                        invoiceBillingAddress: {
        
                        },
                        // Items Header
                        itemsHeader: {
                            margin: [0, 5, 0, 5],
                            bold: true
                        },
                        // Item Title
                        itemTitle: {
                            bold: true,
                        },
                        itemSubTitle: {
                            italics: true,
                            fontSize: 11
                        },
                        itemNumber: {
                            margin: [0, 5, 0, 5],
                            alignment: 'center',
                        },
                        itemTotal: {
                            margin: [0, 5, 0, 5],
                            bold: true,
                            alignment: 'center',
                        },
        
                        // Items Footer (Subtotal, Total, Tax, etc)
                        itemsFooterSubTitle: {
                            margin: [0, 5, 0, 5],
                            bold: true,
                            alignment: 'right',
                        },
                        itemsFooterSubValue: {
                            margin: [0, 5, 0, 5],
                            bold: true,
                            alignment: 'center',
                        },
                        itemsFooterTotalTitle: {
                            margin: [0, 5, 0, 5],
                            bold: true,
                            alignment: 'right',
                        },
                        itemsFooterTotalValue: {
                            margin: [0, 5, 0, 5],
                            bold: true,
                            alignment: 'center',
                        },
                        signaturePlaceholder: {
                            margin: [0, 70, 0, 0],
                        },
                        signatureName: {
                            bold: true,
                            alignment: 'center',
                        },
                        signatureJobTitle: {
                            italics: true,
                            fontSize: 10,
                            alignment: 'center',
                        },
                        notesTitle: {
                            fontSize: 10,
                            bold: true,
                            margin: [0, 50, 0, 3],
                        },
                        notesText: {
                            fontSize: 10
                        },
                        center: {
                            alignment: 'center',
                        },
                    },
                    defaultStyle: {
                        columnGap: 20,
                    }
                }
                this.pdfObj = pdfmake.createPdf(dd);
                this.downloadPdf();
              //  pdfmake.createPdf(dd).open();
                
        
            }
    
  downloadPdf() {
    if (this.plt.is('cordova')) {
      this.pdfObj.getBuffer((buffer) => {
        var blob = new Blob([buffer], { type: 'application/pdf' });

        // Save the PDF to the data Directory of our App
        this.file.writeFile(this.file.dataDirectory, 'Invoice.pdf', blob, { replace: true }).then(fileEntry => {
          // Open the PDf with the correct OS tools
          this.fileOpener.open(this.file.dataDirectory + 'Invoice.pdf', 'application/pdf');
        })
      });
    } else {
      // On a browser simply use download!
      this.pdfObj.download();
    }
  }

    makePdfForManualBooking(customerName, customerAddress, customerPhone, date, seats, price) {
        pdfmake.vfs = pdfFonts.pdfMake.vfs;
        var dd = {


            header: {
                columns: [
                    { text: 'HEADER LEFT', style: 'documentHeaderLeft' },
                    { text: 'HEADER CENTER', style: 'documentHeaderCenter' },
                    { text: 'HEADER RIGHT', style: 'documentHeaderRight' }
                ]
            },
            footer: {
                columns: [
                    { text: 'FOOTER LEFT', style: 'documentFooterLeft' },
                    { text: 'FOOTER CENTER', style: 'documentFooterCenter' },
                    { text: 'FOOTER RIGHT', style: 'documentFooterRight' }
                ]
            },
            content: [
                // Header
                {
                    columns: [
                        {
                            image: 'data:image/png;base64,',
                            width: 150
                        },

                        [
                            {
                                text: 'INVOICE',
                                style: 'invoiceTitle',
                                width: '*'
                            },
                            {
                                stack: [
                                    {
                                        columns: [
                                            {
                                                text: 'Invoice #',
                                                style: 'invoiceSubTitle',
                                                width: '*'

                                            },
                                            {
                                                text: '00001',
                                                style: 'invoiceSubValue',
                                                width: 100

                                            }
                                        ]
                                    },
                                    {
                                        columns: [
                                            {
                                                text: 'Date Issued',
                                                style: 'invoiceSubTitle',
                                                width: '*'
                                            },
                                            {
                                                text: 'June 01, 2016',
                                                style: 'invoiceSubValue',
                                                width: 100
                                            }
                                        ]
                                    },
                                    {
                                        columns: [
                                            {
                                                text: 'Due Date',
                                                style: 'invoiceSubTitle',
                                                width: '*'
                                            },
                                            {
                                                // text:'June 05, 2016',
                                                text: date,
                                                style: 'invoiceSubValue',
                                                width: 100
                                            }
                                        ]
                                    },
                                ]
                            }
                        ],
                    ],
                },
                // Billing Headers
                {
                    columns: [
                        {
                            text: 'Billing From',
                            style: 'invoiceBillingTitle',

                        },
                        {
                            text: 'Billing To',
                            style: 'invoiceBillingTitle',

                        },
                    ]
                },
                // Billing Details
                {
                    columns: [
                        {
                            //  text: 'Your Name \n Your Company Inc.',
                            text: this.invoiceServiceProvider.companyName,
                            style: 'invoiceBillingDetails'
                        },
                        {
                            //  text: 'Client Name \n Client Company',
                            text: customerName,
                            style: 'invoiceBillingDetails'
                        },
                    ]
                },
                // Billing Address Title
                {
                    columns: [
                        {
                            text: 'Address',
                            style: 'invoiceBillingAddressTitle'
                        },
                        {
                            text: 'Address',
                            style: 'invoiceBillingAddressTitle'
                        },
                    ]
                },
                // Billing Address
                {
                    columns: [
                        {
                            //  text: '9999 Street name 1A \n New-York City NY 00000 \n   USA',
                            text: this.invoiceServiceProvider.officeLocation,
                            style: 'invoiceBillingAddress'
                        },
                        {
                            //  text: '1111 Other street 25 \n New-York City NY 00000 \n   USA',
                            text: customerAddress,
                            style: 'invoiceBillingAddress'
                        },
                    ]
                },
                // Line breaks
                '\n\n',
                // Items
                {
                    table: {
                        // headers are automatically repeated if the table spans over multiple pages
                        // you can declare how many rows should be treated as headers
                        headerRows: 1,
                        widths: ['*', 40, 'auto', 40, 'auto', 80],

                        body: [
                            // Table Header
                            [
                                {
                                    text: 'Seats',
                                    style: 'itemsHeader'
                                },
                                {
                                    text: 'Qty',
                                    style: ['itemsHeader', 'center']
                                },
                                {
                                    text: 'Price',
                                    style: ['itemsHeader', 'center']
                                },
                                {
                                    text: 'Tax',
                                    style: ['itemsHeader', 'center']
                                },
                                {
                                    text: 'Discount',
                                    style: ['itemsHeader', 'center']
                                },
                                {
                                    text: 'Total',
                                    style: ['itemsHeader', 'center']
                                }
                            ],
                            // Items
                            // Item 1
                            [
                                [
                                    {
                                        //  text: 'Item 1',
                                        text: seats,
                                        style: 'itemTitle'
                                    },
                                    {
                                        text: 'Item Description',
                                        style: 'itemSubTitle'

                                    }
                                ],
                                {
                                    //  text:'1',
                                    text: seats.length,
                                    style: 'itemNumber'
                                },
                                {
                                    //text: '$999.99',
                                    text: '$300.00',
                                    style: 'itemNumber'
                                },
                                {
                                    text: '0%',
                                    style: 'itemNumber'
                                },
                                {
                                    text: '0%',
                                    style: 'itemNumber'
                                },
                                {
                                    //  text: '$999.99',
                                    text: price,
                                    style: 'itemTotal'
                                }
                            ],
                            // Item 2
                            //  [ 
                            //      [
                            //          {
                            //              text: 'Item 2',
                            //              style:'itemTitle'
                            //          }, 
                            //          {
                            //              text: 'Item Description',
                            //              style:'itemSubTitle'

                            //          }
                            //      ], 
                            //      {
                            //          text:'1',
                            //          style:'itemNumber'
                            //      }, 
                            //      {
                            //          text:'$999.99',
                            //          style:'itemNumber'
                            //      }, 
                            //      {
                            //          text:'0%',
                            //          style:'itemNumber'
                            //      }, 
                            //      {
                            //          text: '0%',
                            //          style:'itemNumber'
                            //      },
                            //      {
                            //          text: '$999.99',
                            //          style:'itemTotal'
                            //      } 
                            //  ],
                            // END Items
                        ]
                    }, // table
                    //  layout: 'lightHorizontalLines'
                },
                // TOTAL
                {
                    table: {
                        // headers are automatically repeated if the table spans over multiple pages
                        // you can declare how many rows should be treated as headers
                        headerRows: 0,
                        widths: ['*', 80],

                        body: [
                            // Total
                            [
                                {
                                    text: 'Subtotal',
                                    style: 'itemsFooterSubTitle'
                                },
                                {
                                    //  text:'$2000.00',
                                    text: price,
                                    style: 'itemsFooterSubValue'
                                }
                            ],
                            [
                                {
                                    text: 'Tax 21%',
                                    style: 'itemsFooterSubTitle'
                                },
                                {
                                    text: '$523.13',
                                    style: 'itemsFooterSubValue'
                                }
                            ],
                            [
                                {
                                    text: 'TOTAL',
                                    style: 'itemsFooterTotalTitle'
                                },
                                {
                                    //  text: '$2523.93',
                                    text: price + 523.13,
                                    style: 'itemsFooterTotalValue'
                                }
                            ],
                        ]
                    }, // table
                    layout: 'lightHorizontalLines'
                },
                // Signature
                {
                    columns: [
                        {
                            text: '',
                        },
                        {
                            stack: [
                                {
                                    text: '_________________________________',
                                    style: 'signaturePlaceholder'
                                },
                                {
                                    //  text: 'Your Name',
                                    text: this.invoiceServiceProvider.companyName,
                                    style: 'signatureName'

                                },
                                {
                                    //  text: 'Your job title',
                                    text: 'Manager',
                                    style: 'signatureJobTitle'

                                }
                            ],
                            width: 180
                        },
                    ]
                },
                {
                    text: 'NOTES',
                    style: 'notesTitle'
                },
                {
                    text: 'Some notes goes here \n Notes second line',
                    style: 'notesText'
                }
            ],
            styles: {
                // Document Header
                documentHeaderLeft: {
                    fontSize: 10,
                    margin: [5, 5, 5, 5],
                    alignment: 'left'
                },
                documentHeaderCenter: {
                    fontSize: 10,
                    margin: [5, 5, 5, 5],
                    alignment: 'center'
                },
                documentHeaderRight: {
                    fontSize: 10,
                    margin: [5, 5, 5, 5],
                    alignment: 'right'
                },
                // Document Footer
                documentFooterLeft: {
                    fontSize: 10,
                    margin: [5, 5, 5, 5],
                    alignment: 'left'
                },
                documentFooterCenter: {
                    fontSize: 10,
                    margin: [5, 5, 5, 5],
                    alignment: 'center'
                },
                documentFooterRight: {
                    fontSize: 10,
                    margin: [5, 5, 5, 5],
                    alignment: 'right'
                },
                // Invoice Title
                invoiceTitle: {
                    fontSize: 22,
                    bold: true,
                    alignment: 'right',
                    margin: [0, 0, 0, 15]
                },
                // Invoice Details
                invoiceSubTitle: {
                    fontSize: 12,
                    alignment: 'right'
                },
                invoiceSubValue: {
                    fontSize: 12,
                    alignment: 'right'
                },
                // Billing Headers
                invoiceBillingTitle: {
                    fontSize: 14,
                    bold: true,
                    alignment: 'left',
                    margin: [0, 20, 0, 5],
                },
                // Billing Details
                invoiceBillingDetails: {
                    alignment: 'left'

                },
                invoiceBillingAddressTitle: {
                    margin: [0, 7, 0, 3],
                    bold: true
                },
                invoiceBillingAddress: {

                },
                // Items Header
                itemsHeader: {
                    margin: [0, 5, 0, 5],
                    bold: true
                },
                // Item Title
                itemTitle: {
                    bold: true,
                },
                itemSubTitle: {
                    italics: true,
                    fontSize: 11
                },
                itemNumber: {
                    margin: [0, 5, 0, 5],
                    alignment: 'center',
                },
                itemTotal: {
                    margin: [0, 5, 0, 5],
                    bold: true,
                    alignment: 'center',
                },

                // Items Footer (Subtotal, Total, Tax, etc)
                itemsFooterSubTitle: {
                    margin: [0, 5, 0, 5],
                    bold: true,
                    alignment: 'right',
                },
                itemsFooterSubValue: {
                    margin: [0, 5, 0, 5],
                    bold: true,
                    alignment: 'center',
                },
                itemsFooterTotalTitle: {
                    margin: [0, 5, 0, 5],
                    bold: true,
                    alignment: 'right',
                },
                itemsFooterTotalValue: {
                    margin: [0, 5, 0, 5],
                    bold: true,
                    alignment: 'center',
                },
                signaturePlaceholder: {
                    margin: [0, 70, 0, 0],
                },
                signatureName: {
                    bold: true,
                    alignment: 'center',
                },
                signatureJobTitle: {
                    italics: true,
                    fontSize: 10,
                    alignment: 'center',
                },
                notesTitle: {
                    fontSize: 10,
                    bold: true,
                    margin: [0, 50, 0, 3],
                },
                notesText: {
                    fontSize: 10
                },
                center: {
                    alignment: 'center',
                },
            },
            defaultStyle: {
                columnGap: 20,
            }
        }
        pdfmake.createPdf(dd).open();
    }
}
interface customer {
    _id: string;
    username: string;
    email: string;
    password: string;
    is_deleted: boolean;
    phone: string;
    cnic: string;
}
export interface Booking {
    id: string,
    price: number,
    seats: Array<string>,
    routId: string,
    customerId: string,
    providerId: string,
    timing: string,
    date:string,
    expire: boolean
}